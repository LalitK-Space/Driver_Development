 ...
 * 											stm32f407xx.h
 *
 * Contains all the micro-controller specific details:
 ...
 *
 * 	-- IMPORTANT --
 * 	New folder named "drivers" is created with sub folders "Inc" to keep driver header files
 * 	and "Src" to keep drivers source files.
 *	- In projects properties (C/C++ build), un-check "Exclude resource from project" option.
 *	- Include PATH of folder "drivers" in the ECLIPSE (cubeMX) Build system.
 *	  ( -> Project Properties -> C/C++ Build -> Settings -> MCU GCC Compiler -> Include Path ->
 *	    -> Add new include path of folder -> Workspace -> Select project ->
 *	    -> Select Inc of drivers folder -> OK )
 *
 */
